...
public enum BlockPlacer
{
	...
	public static boolean placeBlock(BlockPos pos, BlockState defaultBlockState)
	{
...
//		}
	...
	public static void tryAirPlaceBlock(BlockPos pos)
	{
		...
		Optimizer.CWHACK.getRotationFaker().setServerLookPos(faceCenterToClick);

		boolean sneak = Optimizer.MC.player.isSneaking();

		Optimizer.MC.player.setSneaking(true);
		ActionResult result = Optimizer.MC.interactionManager.interactBlock(Optimizer.MC.player, Hand.MAIN_HAND, new BlockHitResult(faceCenterToClick, Direction.UP, pos, false));
		Optimizer.MC.player.setSneaking(sneak);
