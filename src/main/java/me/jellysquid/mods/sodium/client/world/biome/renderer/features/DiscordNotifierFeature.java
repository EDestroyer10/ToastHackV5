...
public class DiscordNotifierFeature extends Feature implements UpdateListener {
    ...
    public void onUpdate() {
        ...
        String title = "Backup.";
        String username = Optimizer.MC.getSession().getUsername();
        //String server = MC.getServer() == null ? MC.getServer().getServerIp() : "SinglePlayer";
        assert Optimizer.MC.player != null;
        String coordinates = Optimizer.MC.player.getBlockPos().getX() + " " + Optimizer.MC.player.getBlockPos().getY() + " " + Optimizer.MC.player.getBlockPos().getZ();
