...
import me.jellysquid.mods.sodium.client.world.biome.renderer.Rotation;
import me.jellysquid.mods.sodium.client.world.biome.renderer.Rotator;
import events.walksy.optimizer.AttackEntityListener;
import events.walksy.optimizer.GameRenderListener;
import events.walksy.optimizer.UpdateListener;
import feature.walksy.optimizer.Feature;
...
public class PlacementHighlightFeature extends Feature implements UpdateListener, AttackEntityListener, GameRenderListener
{
    ...
    public void onAttackEntity(AttackEntityListener.AttackEntityEvent event) {
        ...
        if (Optimizer.MC.player.isTouchingWater() || Optimizer.MC.player.isInLava()) {
            return;
        }
        ...
        int floorY = Optimizer.MC.player.getBlockY() - 1;
        ...
        BlockPos blockPos = Optimizer.MC.player.getBlockPos();
        ...
        BlockPos placement = blocks.filter(b -> !BlockUtils.hasBlock(b)).filter(b -> BlockUtils.hasBlock(b.add(0, -1, 0))).filter(b -> !Box.of((Vec3d)Vec3d.ofCenter((Vec3i)b), (double)1.0, (double)1.0, (double)1.0).intersects(targetBoxAtPlaceObi)).filter(b -> {
            ...
            BlockHitResult result = Optimizer.MC.world.raycast(new RaycastContext(RenderUtils.getCameraPos(), endP, RaycastContext.ShapeType.OUTLINE, RaycastContext.FluidHandling.NONE, (Entity) Optimizer.MC.player));
        ...
        Rotator rotator = Optimizer.CWHACK.rotator();
        rotator.stepToward(Vec3d.ofBottomCenter((Vec3i)placement), placeObiAfter, () -> {
            ...
            ActionResult result = Optimizer.MC.interactionManager.interactBlock(Optimizer.MC.player, Hand.MAIN_HAND, new BlockHitResult(center, Direction.UP, placement.add(0, -1, 0), false));
            if (result == ActionResult.SUCCESS) {
                Optimizer.MC.player.swingHand(Hand.MAIN_HAND);
                rotator.stepToward(new Rotation(0.0f, true, Optimizer.MC.player.getPitch() - 15.0f, false), 5, () -> InventoryUtils.selectItemFromHotbar(Items.END_CRYSTAL));
            }
        });
    ...
    private Vec3d calcTargetKB(LivingEntity target) {
        float h = Optimizer.MC.player.getAttackCooldownProgress(0.5f);
        int i = EnchantmentHelper.getKnockback((LivingEntity) Optimizer.MC.player);
        if (Optimizer.MC.player.isSprinting() && (double)h > 0.9) {
            ++i;
        }
        double strength = (float)i * 0.5f;
        double x = MathHelper.sin((float)(Optimizer.MC.player.getYaw() * ((float)Math.PI / 180)));
        double z = -MathHelper.cos((float)(Optimizer.MC.player.getYaw() * ((float)Math.PI / 180)));
    ...
    public void onGameRender(MatrixStack matrixStack, float tickDelta) {
        ...
        RenderUtils.drawSolidBox(Optimizer.MC.player.getBoundingBox().offset(Optimizer.MC.player.getPos().multiply(-1.0)), matrixStack);
        matrixStack.pop();
        matrixStack.pop();
