/**
 * A class to represent Color2s and their respective functions.
 */
...
public class Color2 {
	...
	/**
	 * Color2 Constructor using RGB color space.
	 *
	 * @param r Red component of a Color2.
	 * @param g Green component of a Color2.
	 * @param b Blue component of a Color2.
	 */
	...
	/**
	 * Color2 Constructor using HSV color space.
	 *
	 * @param hue
	 * @param saturation
	 * @param luminance
	 */
	public Color2(float hue, float saturation, float luminance) {
	...
	public void setHSV(float hue, float saturation, float luminance) {
		...
		Color2 vec = hsv2rgb(hue, saturation, luminance);
		this.r = vec.r;
		this.g = vec.g;
		this.b = vec.b;
	}
	...
	/**
	 * Returns the Color2 as a string in format RRRGGGBBB.
	 *
	 * @return
	 */
	 ...
	 */
	public String getColor2AsHex() {
		return String.format("#%06X", this.getColor2AsInt());
	}
	 ...
	 *
	 * @param hex Color2 as Hex.
	 * @return Color2 from Hex Code.
	 */
	public static Color2 convertHextoRGB(String hex) {
	 ...
	 */
	public static Color2 hsv2rgb(float hue, float saturation, float luminance) {
		// Get the side that the colour is contained in.
		float h = (hue / 60);
		float chroma = luminance * saturation;
		float x = chroma * (1 - Math.abs((h % 2) - 1));

		// Depending on the side, set the Chroma component to the correct Color2.
		Vector3f rgbVec;
		if (h >= 0 && h <= 1) {
		...
		// If the Color2 does exist, add luminance and convert to RGB.
		if (rgbVec != null) {
			float m = luminance - chroma;
			return new Color2((int) (255.0f * (rgbVec.x + m)), (int) (255.0f * (rgbVec.y + m)),
