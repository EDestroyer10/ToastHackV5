...
public class GlyphPageFontRenderer {
	...
	public int drawStringWithShadow(DrawContext context, String text, double x, double y, int color) {
	...
	public int drawString(DrawContext context, String text, double x, double y, int color) {
		return drawString(context, text, (float) x, (float) y, color, false);
	}

	public int drawCenteredString(DrawContext context, String text, double x, double y, float scale, int color) {
		return drawString(context, text, (float) x - getStringWidth(text) / 2, (float) y, scale, color, false);
	}

	public int drawCenteredString(DrawContext context, String text, double x, double y, int color) {
		return drawString(context, text, (float) x - getStringWidth(text) / 2, (float) y, color, false);
	}

	public int drawCenteredStringWidthShadow(DrawContext context, String text, double x, double y, int color) {
		return drawString(context, text, (float) x - getStringWidth(text) / 2, (float) y, color, true);
	}

	public int drawString(DrawContext context, String text, float x, float y, float scale, int color, boolean dropShadow) {
		this.resetStyles();
		int i;

		if (dropShadow) {
			i = this.renderString(context.getMatrices(), text, x + 1.0F, y + 1.0F, scale, color, true);
			i = Math.max(i, this.renderString(context.getMatrices(), text, x, y, scale, color, false));
		} else {
			i = this.renderString(context.getMatrices(), text, x, y, scale, color, false);
		}
	 ...
	 */
	public int drawString(DrawContext context, String text, float x, float y, int color, boolean dropShadow) {
		this.resetStyles();
		int i;

		if (dropShadow) {
			i = this.renderString(context.getMatrices(), text, x + 1.0F, y + 1.0F, color, true);
			i = Math.max(i, this.renderString(context.getMatrices(), text, x, y, color, false));
		} else {
			i = this.renderString(context.getMatrices(), text, x, y, color, false);
		}
	...
	private int renderString(MatrixStack matrices, String text, float x, float y, float scale, int color, boolean dropShadow) {
		if (text == null) {
			return 0;
		} else {
	...
	private void renderStringAtPos(MatrixStack matrices, String text, float scale, boolean shadow, int color) {
		GlyphPage glyphPage = getCurrentGlyphPage();
		float alpha = (float) (color >> 24 & 255) / 255.0F;
		float g = (float) (color >> 16 & 255) / 255.0F;
		float h = (float) (color >> 8 & 255) / 255.0F;
