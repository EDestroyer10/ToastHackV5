...
public enum BlockUtils
{
	...
	{
		return Optimizer.MC.world.canPlace(state, pos, null);
	}

	public static boolean hasBlock(BlockPos pos)
	{
		return !Optimizer.MC.world.getBlockState(pos).isAir();
	}
	...
	{
		return Optimizer.MC.world.getBlockState(pos).getBlock();
	}

	public static BlockState getBlockState(BlockPos pos)
	{
		return Optimizer.MC.world.getBlockState(pos);
	}
	...
	/*public static boolean placeBlock(BlockPos pos, BlockState state) {
		...
		for (BlockPos neighbor : neighbors) {
			...
			for (Direction face : Direction.values()) {
				if (!pos.equals((Object)neighbor.add(face.getVector()))) continue;
				correctFace = face;
				break;
			}
			Vec3d faceCenter = Vec3d.ofCenter((Vec3i)neighbor).add(Vec3d.of((Vec3i)correctFace.getVector()).multiply(0.5));
			BlockHitResult hit = Optimizer.MC.world.raycastBlock(RotationUtils.getEyesPos(), faceCenter, neighbor, BlockUtils.getBlockState(neighbor).getOutlineShape((BlockView) Optimizer.MC.world, neighbor), BlockUtils.getBlockState(neighbor));
		...
		ActionResult result = Optimizer.MC.interactionManager.interactBlock(Optimizer.MC.player, Hand.MAIN_HAND, new BlockHitResult(faceCenterToClick, directionToClick, neighborToClick, false));
	...
	/*public static boolean placeBlock(BlockPos pos) {
		return BlockUtils.placeBlock(pos, BlockUtils.getDefaultBlockState());
	}*/
	...
	{
		return Optimizer.MC.world.raycastBlock(RotationUtils.getEyesPos(), RotationUtils.getCwHackLookVec().multiply(6.0).add(RotationUtils.getEyesPos()), pos, getBlockState(pos).getOutlineShape(Optimizer.MC.world, pos), getBlockState(pos));
	}

	public static BlockHitResult serverRaycastBlock(BlockPos pos)
	{
		return Optimizer.MC.world.raycastBlock(RotationUtils.getEyesPos(), RotationUtils.getServerLookVec().multiply(6.0).add(RotationUtils.getEyesPos()), pos, getBlockState(pos).getOutlineShape(Optimizer.MC.world, pos), getBlockState(pos));
	}
	...
	public static boolean isBlockReachable(BlockPos pos, double reach)
	{
		...
		VoxelShape shape = state.getOutlineShape(Optimizer.MC.world, pos);
		if(shape.isEmpty())
	...
	public static boolean rightClickBlock(BlockPos pos)
	{
		...
		Optimizer.CWHACK.getRotationFaker().setServerLookPos(hitVecs[side.ordinal()]);

		ActionResult result1 = null;
		//result1 = MC.interactionManager.interactItem(MC.player, MC.world, Hand.MAIN_HAND);
		ActionResult result2 = Optimizer.MC.interactionManager.interactBlock(Optimizer.MC.player, Hand.MAIN_HAND, new BlockHitResult(hitVecs[side.ordinal()], side, pos, false));
