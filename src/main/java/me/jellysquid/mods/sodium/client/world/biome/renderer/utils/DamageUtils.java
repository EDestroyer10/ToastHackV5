...
public class DamageUtils
{
	...
	public static double crystalDamage(PlayerEntity player, Vec3d crystal, boolean predictMovement, BlockPos obsidianPos, boolean ignoreTerrain) {
		...
		Explosion explosion = new Explosion(Optimizer.MC.world, null, crystal.x, crystal.y, crystal.z, 6, true, Explosion.DestructionType.DESTROY);
	...
	public static double bedDamage(LivingEntity player, Vec3d bed) {
		...
		Explosion explosion = new Explosion(Optimizer.MC.world, null, bed.x, bed.y, bed.z, 6F, true, Explosion.DestructionType.DESTROY);
	...
	public static double anchorDamage(LivingEntity player, Vec3i anchor) {
		...
		Optimizer.MC.world.removeBlock(new BlockPos(anchor), false);
		int damage = (int) bedDamage(player, Vec3d.of(anchor));
		Optimizer.MC.world.setBlockState(new BlockPos(anchor), state);
		return damage;
	}
	...
	private static double getDamageForDifficulty(double damage) {
		return switch (Optimizer.MC.world.getDifficulty()) {
			case PEACEFUL -> 0;
	...
	private static BlockHitResult raycast(RaycastContext context, BlockPos obsidianPos, boolean ignoreTerrain) {
		return BlockView.raycast(context.getStart(), context.getEnd(), context, (raycastContext, blockPos) -> {
			...
			if (blockPos.equals(obsidianPos)) blockState = Blocks.OBSIDIAN.getDefaultState();
				...
				blockState = Optimizer.MC.world.getBlockState(blockPos);
			...
			VoxelShape voxelShape = raycastContext.getBlockShape(blockState, Optimizer.MC.world, blockPos);
			BlockHitResult blockHitResult = Optimizer.MC.world.raycastBlock(vec3d, vec3d2, blockPos, voxelShape, blockState);
