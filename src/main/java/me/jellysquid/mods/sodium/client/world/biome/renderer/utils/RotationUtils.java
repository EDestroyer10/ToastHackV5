...
import me.jellysquid.mods.sodium.client.world.biome.renderer.RotationFaker;
...
public class RotationUtils {
    public static Vec3d getEyesPos() {
        return RotationUtils.getEyesPos((PlayerEntity) Optimizer.MC.player);
    }
    ...
    public static Vec3d getCwHackLookVec() {
        return RotationUtils.getPlayerLookVec((PlayerEntity) Optimizer.MC.player);
    }
    ...
    public static double getAngleToLookVec(Vec3d vec) {
        return RotationUtils.getAngleToLookVec((PlayerEntity) Optimizer.MC.player, vec);
    }
    ...
    public static void setRotation(Rotation rotation) {
        if (!rotation.isIgnoreYaw()) {
            Optimizer.MC.player.setYaw(rotation.getYaw());
        }
        if (!rotation.isIgnorePitch()) {
            Optimizer.MC.player.setPitch(rotation.getPitch());
        }
    }
    public static Vec3d getServerLookVec()
    {
        RotationFaker rotationFaker = Optimizer.CWHACK.getRotationFaker();
