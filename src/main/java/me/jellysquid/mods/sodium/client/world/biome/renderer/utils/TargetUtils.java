...
public enum TargetUtils
{
	...
	{
		return StreamSupport.stream(Optimizer.MC.world.getEntities().spliterator(), true)
				.filter(e -> e != Optimizer.MC.player)
				...
				.filter(e -> !(config.filterTamed && !(e instanceof TameableEntity && ((TameableEntity) e).isTamed() || e instanceof AbstractHorseEntity && ((AbstractHorseEntity) e).isTame())))
	...
	public static class TargetFilterConfiguration
	{
		...
		public enum Type
		{
			...
			DISTANCE(Comparator.comparingDouble(e -> e.squaredDistanceTo(Optimizer.MC.player))),
